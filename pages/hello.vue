...
export default {
  ...
  props: ['end'],
  computed: {
    ...mapState(['animationItems', 'displayState', 'fontSize', 'w']),
    screenSize() {
      return {
  ...
  mounted() {
    ...
    this.setFontSize(this.screenSize)
    this.animateHello()
  },
  updated() {
    this.setFontSize(this.screenSize)
  ...
  methods: {
    ...mapActions(['setFontSize', 'setDisplay']),
    animateHello() {
    ...
    changeItem(item) {
      ...
      this.setDisplay({ item, display: 'inline-block' })
    },
  },
}
</script>
